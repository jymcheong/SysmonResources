...
* After that, there will be 3 IExplorer.exe processes created. There are all invisible but yet none of these browser processes are child to the initial Powershell. 
* I won't be going into details with the code-execution sequences but will focus on a key difference between Empire C2 behavior.
* Now if you filter the log to only look at Event ID 3, you will find that there are only 3 events that indicated communications to Kali host. I also let it run much longer but not capture in the logs uploaded. **There is NO repeated Event ID 3 events from Powershell or these 3 IExplorer processes.** 
